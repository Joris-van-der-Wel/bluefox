...
describe('Waiting for a single selector', {timeout: 60000}, () => {
    afterEach(async () => {
        await closeWindow();
    });
    ...
    specify('single mutation', async () => {
        ...
        const waitExpression = bluefox.target(window).timeout('5s').selector('body > p strong em.foo.bar');
    ...
    specify('Case: MP', {timeout: CASE_MP_ITERATIONS * 10000, slow: CASE_MP_ITERATIONS * 4000}, async () => {
        ...
        const waitExpressions = [
            ...
            baseExpression.selector('body > section > #async-content-foo em.async-content-foo-1'),
