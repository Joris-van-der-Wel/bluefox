...
describe('Waiting for a XPath Expression', {slow: 2000, timeout: 10000}, () => {
    beforeEach(async () => {
    });
    ...
    it('Should wait for an element added after a delay', async () => {
        ...
        await run(async ({window, Bluefox, reportProgress}) => {
            await new Bluefox().target(window).timeout('4s').xpath('.//strong[./@id = \'modification\']');
    ...
    it('Should wait for an element added deep in the tree after a delay', async () => {
        ...
        await run(async ({window, Bluefox, reportProgress}) => {
            await new Bluefox().target(window).timeout('4s').xpath('.//body//ul/li[./@id = \'modification\']');
    ...
    it('Should wait for a changed Text node', async () => {
        ...
        await run(async ({window, Bluefox, reportProgress}) => {
            await new Bluefox().target(window).timeout('4s').xpath('.//*[./@id = \'secondP\' and contains(text(),\'Replaced the text\')]');
            reportProgress('after wait');
        });
        const progress = await getProgress();
    ...
    it('Should wait for an element removed after a delay', async () => {
        ...
        await run(async ({window, Bluefox, reportProgress}) => {
            await new Bluefox().target(window).timeout('4s').documentInteractive().xpath('.//*[./@id = \'thirdP\']').amount(0);
            reportProgress('after wait');
        });
        const progress = await getProgress();
    ...
    it('Should wait for an element removed deep in the tree after a delay', async () => {
        ...
        await run(async ({window, Bluefox, reportProgress}) => {
            await new Bluefox().target(window).timeout('4s').documentInteractive().xpath('.//body//ul/*[./@id = \'firstLi\']').amount(0);
            reportProgress('after wait');
        });
        const progress = await getProgress();
    ...
    it('Should wait for an attribute set after a delay', async () => {
        ...
        await run(async ({window, Bluefox, reportProgress}) => {
            await new Bluefox().target(window).timeout('4s').xpath(
